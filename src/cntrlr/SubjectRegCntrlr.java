...
@Controller
public class SubjectRegCntrlr {
	@RequestMapping("/jsp/subjectregister")
 ...
 YearClassDivSubMasterDao master= (YearClassDivSubMasterDao) r.getBean("YearClassDivSubMasterDao");
		...
		SubjectDao subjectDao = (SubjectDao) r.getBean("SubjectDao");
		...
		String [] subjMrk=request.getParameterValues("total");
		String [] scored=request.getParameterValues("scored");
		String message;
		try{
		String masterstr;
		...
		int i=0;
		Map<Integer,Subject> markInSubject= new TreeMap<Integer,Subject>();
		while(!subj[i].equals("") && !subjMrk[i].equals("") && !scored[i].equals(""))
		...
		{	int j=i+1;
			...
			markInSubject.put(Integer.parseInt(scored[i]), temp1);
		...
		student.setStudentsRollNo(Integer.parseInt(rno));
		student.setName(name);
	
		
		student.setMarkInSubject(markInSubject);
